...
public class CropImageView extends TransformImageView {
    ...
    public RectF getCropRect(){
        ...
        RectF imageCorners=RectUtils.trapToRect(mCurrentImageCorners);
        float left=imageCorners.left,top=imageCorners.top;

        RectF result=new RectF(mCropRect.left-left,mCropRect.top-top,mCropRect.right-left,mCropRect.bottom-top);


        int[] origSizes=getBitmapSizes(getImageInputPath());
        float scaleX=origSizes[0]/imageCorners.width(),scaleY=origSizes[1]/imageCorners.height();
    ...
    /*sorry for duplicate, but we need it here*/
    public int[] getBitmapSizes(String path){
        ...
        int orientation=getExifInfo().getExifOrientation();
    ...
    public void setInitialCropRect(RectF initial){
        mInitialRectF=initial;
    }
    ...
    private void calculateImageScaleBounds(float drawableWidth, float drawableHeight) {
        float widthScale = mCropRect.width() / drawableWidth/*Math.min(mCropRect.width() / drawableWidth, mCropRect.width() / drawableHeight)*/;
        float heightScale = mCropRect.height() / drawableHeight/*Math.min(mCropRect.height() / drawableHeight, mCropRect.height() / drawableWidth)*/;
    ...
    private void setupInitialImagePosition(float drawableWidth, float drawableHeight) {
        ...
        float left=(mCropRect.left*mInitialRectF.right-mCropRect.right*mInitialRectF.left)/mInitialRectF.width();
        float top = (mCropRect.top*mInitialRectF.bottom-mCropRect.bottom*mInitialRectF.top)/mInitialRectF.height();
        float right= origSizes[0]*(mCropRect.right-left)/mInitialRectF.right+left;
        float bottom=origSizes[1]*(mCropRect.bottom-top)/mInitialRectF.bottom+top;
        RectF resultImageCorners=new RectF(left,top,right,bottom);
        ...
        RectF currentImageCorners=RectUtils.trapToRect(mCurrentImageCorners);

        float scaleX=resultImageCorners.width()/currentImageCorners.width(),scaleY=resultImageCorners.height()/currentImageCorners.height();

        if(scaleX!=1||scaleY!=1){
            mCurrentImageMatrix.postScale(scaleX,scaleY);
            setImageMatrix(mCurrentImageMatrix);
            currentImageCorners=RectUtils.trapToRect(mCurrentImageCorners);
        }
        if(resultImageCorners.left!=currentImageCorners.left||resultImageCorners.top!=currentImageCorners.top) {
            mCurrentImageMatrix.postTranslate(resultImageCorners.left - currentImageCorners.left, resultImageCorners.top - currentImageCorners.top);
            setImageMatrix(mCurrentImageMatrix);
        }
    }
