...
public class AnimDownloadProgressButton extends TextView {
    ...
    private String mProgressTemplate;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mIdleText = a.getString(R.styleable.AnimDownloadProgressButton_progressbtn_text);
        if(!TextUtils.isEmpty(mIdleText)){
            mCurrentText=mIdleText;
        }
        mAboveTextSize=a.getDimensionPixelSize(R.styleable.AnimDownloadProgressButton_android_textSize, getResources().getDimensionPixelSize(R.dimen.label_text_size));
        mProgressTemplate=a.getString(R.styleable.AnimDownloadProgressButton_progressbtn_progress_template);
        a.recycle();
    }
    ...
    public void setIdleText(@StringRes int textId){
        setIdleText(getContext().getString(textId));
    }

    public void setIdleText(CharSequence charSequence){
        mIdleText=charSequence!=null?charSequence.toString():"";
        if(getState()==NORMAL){
            mCurrentText=mIdleText;
        }
        invalidate();
    ...
    public void setProgressText(String text, float progress) {
        mProgressTemplate=text;
        if (progress >= mMinProgress && progress <= mMaxProgress) {
            mCurrentText = String.format(mProgressTemplate, (int) progress);
            ...
            mProgress = 100;
            mCurrentText = String.format(text, (int) mProgress);
            invalidate();
        }
    }
    ...
    public void toProgress(float progress){
        setProgressText(mProgressTemplate,progress);
    }
